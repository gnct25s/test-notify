...
async function main() {
  ...
  if (!fs.existsSync(filePath)) {
    ...
    message = generateErrorMessage(targetDate, "Schedule File was not found");
    await sendToLine(message);
    return;
  }

  const data = JSON.parse(fs.readFileSync(filePath, "utf-8"));
  message = generateMessage(isMorning, targetDate, data);
  console.log(message);
  await sendToLine(message);
}

function generateErrorMessage(_targetDate, _error) {
  let message = "";
